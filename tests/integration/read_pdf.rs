...
fn pdf_image_required_carries_structured_retry_parameters_over_real_stdio() {
    ...
    let response = session.call_tool(2, "read", json!({ "path": "scan.pdf" }));
...
fn pdf_text_read_reports_its_source_id_over_real_stdio() {
    ...
    let response = session.call_tool(2, "read", json!({ "path": "document.pdf" }));
    assert_eq!(response["result"]["isError"], false);
    ...
    let matching = session.call_tool(
        4,
        "read",
        json!({
            "path": "document.pdf",
