#[cfg(test)]
thread_local! {
    pub(super) static BEFORE_READ_HOOK: std::cell::RefCell<Option<Box<dyn FnMut()>>> = const { std::cell::RefCell::new(None) };
    pub(super) static AFTER_READ_HOOK: std::cell::RefCell<Option<Box<dyn FnMut()>>> = const { std::cell::RefCell::new(None) };
}

#[cfg(test)]
pub(super) fn run_before_read_hook() {
...
#[cfg(test)]
pub(super) fn run_after_read_hook() {
...
#[cfg(test)]
pub(super) fn forced_runtime_limit() -> Option<std::time::Duration> {
...
#[cfg(test)]
pub(super) fn take_forced_change() -> bool {
    FORCED_CHANGES
        .fetch_update(
