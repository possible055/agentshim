...
#[cfg(unix)]
pub(crate) type SortKey = Vec<u8>;
#[cfg(windows)]
pub(crate) type SortKey = Vec<u16>;
...
#[cfg(unix)]
pub(crate) fn sort_key(path: &Path) -> SortKey {
...
pub(crate) fn sort_key_byte_len(key: &SortKey) -> usize {
    #[cfg(unix)]
    {
        key.len()
    }
...
pub(crate) fn sort_key_capacity_bytes(key: &SortKey) -> usize {
    #[cfg(unix)]
    {
        key.capacity()
    }
...
#[cfg(unix)]
pub(crate) fn reject_nul(path: &Path) -> Result<(), PathError> {
    use std::os::unix::ffi::OsStrExt;
...
#[cfg(unix)]
pub(crate) fn relative_from_absolute(root: &Path, absolute: &Path) -> Result<PathBuf, PathError> {
    absolute
        .strip_prefix(root)
...
#[cfg(windows)]
pub(crate) fn validate_ambient_path(path: &Path) -> Result<(), PathError> {
    use std::path::Prefix;
...
#[cfg(windows)]
pub(crate) fn windows_component_eq(left: Component<'_>, right: Component<'_>) -> bool {
    use std::ffi::OsStr;
    use std::os::windows::ffi::OsStrExt;
    use std::path::Prefix;
...
#[cfg(windows)]
pub(crate) fn validate_root(path: &Path) -> std::io::Result<()> {
    use std::os::windows::ffi::OsStrExt;
