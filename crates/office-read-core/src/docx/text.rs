...
impl DocxDocument {
    pub(crate) fn markdown_header_footer_units(&self) -> (Vec<String>, Vec<String>) {
        let context = MarkdownCtx {
            styles: self.styles.as_ref(),
            numbering: self.numbering.as_ref(),
        };
        split_headers_footers(self, &context)
    }

    pub(crate) fn markdown_body_block(&self, index: usize) -> Option<String> {
        let element = self.body.elements.get(index)?;
        let context = MarkdownCtx {
            styles: self.styles.as_ref(),
            numbering: self.numbering.as_ref(),
        };
        let mut markdown = String::new();
        markdown_blocks(std::slice::from_ref(element), &context, &mut markdown, 0);
        Some(markdown)
    }
            ...
            BlockElement::Table(table) => {
                for row in &table.rows {
                    for (index, cell) in row.cells.iter().enumerate() {
                        ...
                        plain_text_blocks(&cell.content, out);
                    }
                    out.push('\n');
                }
            }
