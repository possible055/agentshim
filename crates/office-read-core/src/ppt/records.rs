...
impl<'a> Iterator for RecordIter<'a> {
    type Item = Result<PptRecord<'a>>;

    fn next(&mut self) -> Option<Self::Item> {
        if let Err(error) =
            crate::budget::check_cancelled().and_then(|()| crate::budget::charge_ppt_record())
        {
        ...
        let Some(header_end) = self.pos.checked_add(8) else {
            self.pos = self.data.len();
            return Some(Err(PptError::InvalidRecord(
        ...
        let Some(data_end) = data_start.checked_add(header.rec_len as usize) else {
            self.pos = self.data.len();
            return Some(Err(PptError::InvalidRecord(
...
fn map_budget_error(error: crate::OfficeReadError) -> PptError {
    match error {
        ...
        crate::OfficeReadError::Invalid { .. } | crate::OfficeReadError::Unsupported { .. } => {
            PptError::InvalidRecord("resource validation failed".to_owned())
        }
    }
}
