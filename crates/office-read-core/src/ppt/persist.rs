...
pub fn build(stream: &[u8], current_user: Option<&[u8]>) -> Result<Option<PersistDirectory>> {
    let current_start = current_user
        .and_then(offset_to_current_edit)
        .filter(|&off| user_edit_atom_at(stream, off).is_some());
    let start = match current_start {
        Some(start) => Some(start),
        None => find_latest_user_edit_atom(stream)?,
    };
    ...
    for _ in 0..MAX_EDIT_CHAIN_LEN {
        ...
        if !seen.insert(off) {
            return Err(PptError::Corrupted("cyclic edit chain".to_owned()));
        }

        let edit = user_edit_atom_at(stream, off)
            .ok_or_else(|| PptError::Corrupted("invalid edit chain offset".to_owned()))?;
        if doc_persist_id.is_none() {
            doc_persist_id = Some(edit.doc_persist_id_ref);
        }

        let entries = persist_directory_entries_at(stream, edit.offset_persist_directory)?
            .ok_or_else(|| PptError::Corrupted("invalid persist directory".to_owned()))?;
        ...
        for (id, off) in entries {
            crate::budget::charge_ppt_item("office_ppt_persist_entries")
    ...
    if offset.is_some() {
        return Err(PptError::ResourceLimit {
            resource: "office_ppt_edit_chain",
            limit: MAX_EDIT_CHAIN_LEN as u64,
            observed: MAX_EDIT_CHAIN_LEN as u64 + 1,
        });
    }
...
fn persist_directory_entries_at(stream: &[u8], offset: u32) -> Result<Option<Vec<(u32, u32)>>> {
    let offset = offset as usize;
    let Some(header_bytes) = stream.get(offset..offset.saturating_add(8)) else {
        return Ok(None);
    };
        ...
        for i in 0..c_persist {
            let Some(chunk) = body.get(pos..pos + 4) else {
                return Err(PptError::Corrupted(
                    "truncated persist directory entry".to_owned(),
                ));
            };
            crate::budget::check_model_items(
                "office_ppt_persist_entries",
    ...
    if pos != body.len() {
        return Err(PptError::Corrupted(
            "truncated persist directory header".to_owned(),
        ));
    }
    Ok(Some(entries))
}
...
fn find_latest_user_edit_atom(stream: &[u8]) -> Result<Option<usize>> {
    let mut latest = None;
...
mod tests {
    ...
    fn newer_edit_entry_wins_over_older_edit_for_same_persist_id() {
        ...
        assert_eq!(
            dir.resolve(2),
            Some(99),
            "newest edit's directory entry must win"
        );
    }
