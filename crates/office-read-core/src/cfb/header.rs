    ...
    pub fn parse(buf: &[u8]) -> Result<Self> {
        ...
        if minor_version != 0x003E {
            return Err(CfbError::InvalidHeader(format!(
                "unsupported minor version: {minor_version}"
            )));
        }
        ...
        if (major_version == 3 && sector_power != 9) || (major_version == 4 && sector_power != 12) {
            return Err(CfbError::InvalidHeader(
                "invalid sector size exponent".into(),
            ));
        }
        ...
        if mini_sector_power != 6 {
            return Err(CfbError::InvalidHeader(
                "mini-sector size must be 64".into(),
            ));
        }
        ...
        if major_version == 3
            && u32::from_le_bytes([buf[0x28], buf[0x29], buf[0x2A], buf[0x2B]]) != 0
        {
            return Err(CfbError::InvalidHeader(
                "v3 directory sector count must be zero".into(),
            ));
        }
        if mini_stream_cutoff != 4096 {
            return Err(CfbError::InvalidHeader(
                "mini-stream cutoff must be 4096".into(),
            ));
        }
        if first_dir_sector > MAX_REG_SECT {
            return Err(CfbError::InvalidHeader(
                "invalid first directory sector".into(),
            ));
        }
        if (mini_fat_sector_count == 0) != (first_mini_fat_sector > MAX_REG_SECT) {
            return Err(CfbError::InvalidHeader(
                "mini-FAT count and first sector disagree".into(),
            ));
        }
        if (difat_sector_count == 0) != (first_difat_sector > MAX_REG_SECT) {
            return Err(CfbError::InvalidHeader(
                "DIFAT count and first sector disagree".into(),
            ));
        }
