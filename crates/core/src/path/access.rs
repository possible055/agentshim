...
impl FileAccess {
    ...
    #[cfg(test)]
    pub fn with_codex_roots(root: Arc<RepositoryRoot>, roots: &[&Path]) -> io::Result<Self> {
    ...
    pub fn with_exact_grant(&self, path: &Path) -> Result<Self, PathError> {
        reject_nul(path)?;
        if !path.is_absolute() {
    ...
    pub fn resolve(&self, input: &Path) -> Result<ResolvedPath, PathError> {
        if input.is_absolute()
            && let Some(grant) = &self.exact_grant
        {
            let absolute = normalize_absolute(input)?;
            if &absolute == grant {
                let key = absolute
                ...
                return Ok(ResolvedPath::ambient(absolute, key));
            }
        }
    ...
    pub fn resolve_traversal_entry(
        &self,
        operation_root: &ResolvedPath,
        absolute: &Path,
    ) -> Result<ResolvedPath, PathError> {
    ...
    pub fn open_file_identity(&self, path: &ResolvedPath) -> io::Result<File> {
    ...
    pub fn open_read_same_parent_batch(
        &self,
        paths: &[ResolvedPath],
    ...
    pub fn open_same_parent_reader(
        &self,
        first: &ResolvedPath,
...
pub fn batch_parent(path: &ResolvedPath) -> io::Result<&Path> {
...
impl SameParentReader<'_> {
    pub fn open(&self, path: &ResolvedPath) -> io::Result<File> {
