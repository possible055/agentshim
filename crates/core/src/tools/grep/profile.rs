...
#[cfg(feature = "bench-internals")]
pub struct GrepProfileCounters {
...
impl GrepProfiler {
    ...
    pub fn span(&self, stage: GrepStage) -> GrepSpan<'_> {
    ...
    pub fn set_workload(&self, candidate_count: usize, lanes: usize) {
        let Self::Enabled(counters) = self else {
            return;
        };
        counters
    ...
    pub fn record_legacy_stream(&self) {
        if let Self::Enabled(counters) = self {
            counters
                .legacy_stream_files
                .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    ...
    pub fn record_speculative_lease(&self, requested: usize, granted: bool) {
        let Self::Enabled(counters) = self else {
            return;
        };
    ...
    pub fn record_search_slice(&self) {
        if let Self::Enabled(counters) = self {
            counters
                .search_slice_files
                .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    ...
    pub fn record_search_file(&self, source: GrepSourcePolicy) {
        if let Self::Enabled(counters) = self {
            counters
                .search_file_files
                .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    ...
    pub fn record_searched_candidate(&self) {
        if let Self::Enabled(counters) = self {
            counters
                .searched_candidates
                .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    ...
    pub fn record_matched_candidate(&self) {
        if let Self::Enabled(counters) = self {
            counters
                .matched_candidates
                .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    ...
    pub fn record_reduced_candidate(&self) {
        if let Self::Enabled(counters) = self {
            counters
                .reduced_candidates
                .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    ...
    pub fn record_pathname_reopen(&self) {
        if let Self::Enabled(counters) = self {
            counters
                .pathname_reopens
                .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    ...
    pub fn add_render_copy_bytes(&self, bytes: usize) {
        if let Self::Enabled(counters) = self {
    ...
    pub fn snapshot(&self) -> GrepStageTimings {
        let Self::Enabled(counters) = self else {
...
impl GrepProfiler {
    ...
    pub fn span(&self, _stage: GrepStage) -> GrepSpan {
        let _ = self;
        GrepSpan
    }
    ...
    pub fn record_retry(&self, _capture: bool, _ceiling: usize, _success: bool) {
        let _ = self;
    }
    ...
    pub fn record_search_file(&self, _source: GrepSourcePolicy) {
        let _ = self;
    }
    ...
    pub fn add_render_copy_bytes(&self, _bytes: usize) {
        let _ = self;
    }
}
