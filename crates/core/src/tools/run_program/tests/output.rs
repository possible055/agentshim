...
fn validation_rejects_conflicts_nul_and_oversized_stdin() {
    ...
    assert!(matches!(
        invalid.validate(crate::tools::exec::spawn::default_max_timeout_ms()),
...
fn timeout_report_is_bounded_and_preserves_required_diagnostics() {
    let mut stdout = Capture::new(capture_bytes_per_stream(2, crate::output::MODEL_BYTE_LIMIT));
    stdout.push(&vec![b'o'; crate::output::MODEL_BYTE_LIMIT * 2]);
    let mut stderr = Capture::new(capture_bytes_per_stream(2, crate::output::MODEL_BYTE_LIMIT));
...
fn a_header_near_the_diagnostic_path_limit_still_fits_the_result_budget() {
    for (program, cwd) in [
        ...
        assert!(
            output.fits_budget(&crate::output::TestCallBudget::default()),
...
fn cjk_heavy_process_output_is_bounded_below_the_english_budget() {
    ...
    assert!(english.fits_budget(&crate::output::TestCallBudget::default()));
    assert!(chinese.fits_budget(&crate::output::TestCallBudget::default()));
    assert!(
...
fn timeout_projection_fits_the_complete_error_envelope() {
    ...
    let budget = crate::output::TestCallBudget::default();
    assert!(
        crate::output::tool_result_encoded_len(&report.text, Some(&structured), true)
            <= crate::output::CallBudget::wire_bytes(&budget)
    );
    assert_eq!(
