...
fn validation_rejects_empty_commands_and_unsupported_detach_combinations() {
    ...
    assert!(matches!(
        empty.validate(crate::tools::exec::spawn::default_max_timeout_ms()),
    ...
    assert!(matches!(
        without_log.validate(crate::tools::exec::spawn::default_max_timeout_ms()),
        Err(ProcessError::Validation(_))
    ));
    ...
    assert!(matches!(
        logged.validate(crate::tools::exec::spawn::default_max_timeout_ms()),
        Err(ProcessError::Validation(_))
    ));

    let mut over_limit = request("true");
    over_limit.timeout_ms = Some(default_max_timeout_ms() + 1);
    assert!(matches!(
        over_limit.validate(crate::tools::exec::spawn::default_max_timeout_ms()),
        Err(ProcessError::Validation(_))
    ));

    let mut at_limit = request("true");
    at_limit.timeout_ms = Some(default_max_timeout_ms());
    assert!(
        at_limit
            .validate(crate::tools::exec::spawn::default_max_timeout_ms())
            .is_ok()
...
fn injected_environment_carries_the_probed_locale_and_no_colour_defaults() {
    let plan = bash_environment(&runtime_fixture(None), MsysArgumentConversion::Default);
