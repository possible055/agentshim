...
impl ProcessResolver {
    ...
    #[cfg(test)]
    pub fn for_tests(search_path: Vec<PathBuf>) -> Self {
        Self::from_search_path(search_path)
    }

    pub fn resolve(&self, program: &str, cwd: &Path) -> Result<ResolvedProgram, ProcessError> {
...
#[cfg(windows)]
pub fn launcher_for(path: &Path) -> Result<Launcher, ProcessError> {
    let extension = path
        .extension()
