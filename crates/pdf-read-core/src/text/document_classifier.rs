...
mod tests {
    ...
    fn test_form_field_detection() {
        assert!(DocumentClassifier::contains_form_field_markers(
            "Name: ___________"
        ));
        assert!(DocumentClassifier::contains_form_field_markers(
            "[X] Check here"
    ...
    fn test_academic_characteristics_detection() {
        ...
        assert!(
            stats.math_symbols_count > 2,
            "Should detect multiple math symbols"
        );
        assert!(
            stats.citation_percentage > 0.0,
            "Should detect academic citations"
        );
    }
