    ...
    match char_code {
        ...
        code => {
            if let Some(ch) = char::from_u32(code) {
                ...
                log::warn!(
                    "Character code 0x{:04X} is not a valid Unicode code point",
                    code
                );
                "?".to_string()
            }
...
pub(super) fn font_has_utf8_cmap(font: &FontInfo) -> bool {
...
pub(super) fn get_byte_mode(font: Option<&FontInfo>) -> ByteMode {
...
impl<'a> TextCharIter<'a> {
    pub(super) fn new(bytes: &'a [u8], font: Option<&FontInfo>) -> Self {
        Self {
            bytes,
...
pub(super) fn decode_text_to_unicode(bytes: &[u8], font: Option<&FontInfo>) -> String {
