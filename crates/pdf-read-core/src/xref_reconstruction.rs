...
use scan::find_trailer;
pub(crate) use scan::{ScanChunk, ScanChunks};
...
fn recover_from_objstms<R: Read + Seek>(
    ...
    if let Some(root) = pages_root.or(pages_any) {
        injected.push((
            ObjectRef::new(free_base, 0),
            catalog_dict(ObjectRef::new(root, 0)),
        ));
    ...
    page_objs.sort_unstable();
    log::info!(
        "Recovery: flat /Pages over {} ObjStm-packed pages",
        page_objs.len()
    );
...
pub fn search_nearby_for_object<R: Read + Seek>(
    ...
    log::debug!(
        "Searching for object {} near offset {}",
        obj_id,
        approx_offset
    );
