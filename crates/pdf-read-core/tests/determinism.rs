//! Lossless single-page continuation requires that the same source page render to
//! byte-identical Markdown on every call. Layout and table detection are the paths most
//! likely to leak iteration order or floating-point drift, so this is pinned before any
//! offset-based resume semantics are built on top of it.
...
fn markdown_is_byte_identical_across_processes() {
    ...
    let output = Command::new(executable)
        .args(["--exact", "markdown_digest_child_fixture", "--nocapture"])
        .output()
