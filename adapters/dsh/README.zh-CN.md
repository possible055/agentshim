...
- DSH `0.1.0-rc.6` 至 `0.1.5-rc.1` 套件家族。开发与 CI 使用 DSH `0.1.5-rc.1` 及 Cordis `4.0.2`；peer 声明仍保留历史上兼容的 DSH 版本。
...
源码/原生测试可以通过 `AGENTSHIM_DSH_NATIVE_DLL` 暂存本地构建的库。打包冒烟测试则不同：它会在隔离的临时消费者环境中全新打包并安装入口包及当前平台包，然后通过包导出接口执行原生读取。Linux 矩阵覆盖所有当前可发布的 DSH 套件，并将 `0.1.2-alpha.1` 与 `0.1.3-alpha.1` 标记为不可用；这些历史版本仍保留在 peer 声明中，但不作为可安装的冒烟目标。该目录中现有的 `.tgz` 文件绝不会用作开发或发布的验证证据。
